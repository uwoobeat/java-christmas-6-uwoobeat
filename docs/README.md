...
- 예약은 주문(Order), 적용된 이벤트 목록(Events), 증정품 목록(Giveaways), 금액정보(MoneyInfo), 이벤트배지(Badge)를 가진다.
    - 적용된 이벤트 목록에서 총할인금액을 구할 수 있다.
    - 적용된 이벤트 목록에서 증정품 목록을 구할 수 있다.
    - 증정품 목록에서 총가치금액을 구할 수 있다.
- 금액정보는 총주문금액, 총할인금액, 총혜택금액, 예상결제금액을 가진다.
    - 총주문금액은 주문의 총주문금액이다.
...
- 배지는 이름과 발급 정책을 가진다.
- 배지는 발급 정책을 통해 발급 가능 여부를 판정한다.

### 배지 팩토리(BadgeFactory)

- 배지 팩토리는 현재 존재하는 모든 이벤트 배지를 가진다.
- 다음 배지가 존재한다.
    - 산타(총혜택금액 20,000원 이상)
    - 트리(총혜택금액 10,000원 이상)
    - 별(총혜택금액 5,000원 이상)
- 배지 팩토리에 선언된 순서가 배지의 등급이다.
- 배지 팩토리는 발급 가능한 이벤트 배지 중 가장 높은 등급의 배지를 하나 반환할 수 있다.

### 배지 발급 정책(BadgeIssuePolicy)

- 배지 발급 정책은 금액정보를 통해 발급 가능 여부를 판정한다.
- 배지 발급 정책에는 총혜택금액 기준 발급 정책이 있다.
    - 총혜택금액 기준 발급 정책(OrderBenefitPolicy)
        - 총혜택금액 기준 발급 정책은 기준 총혜택금액을 가진다.
        - 총혜택금액 기준 발급 정책은 금액정보의 총혜택금액이 기준 총혜택금액보다 큰지 검증한다.

### 배지 발급 정책 팩토리(BadgeIssuePolicyFactory)

- 배지 발급 정책 팩토리는 현재 존재하는 모든 배지 발급 정책을 가진다.
- 다음 배지 발급 정책이 존재한다.
    - 총혜택금액 기준 발급 정책 : 산타(20,000원), 트리(10,000원), 별(5,000원)
